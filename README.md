...
Not every seed tells this clean a story. Across 30 seeds, guarded's champion
still ends with a real success rate of exactly zero on 3 of them — guarding
helps a lot, but it is not a guarantee. Every run, including the failures, is
in `guarded_eval/experiments/results.csv`. Nothing is filtered out.
